...
from balatro_rl.engine.descriptions import (
    BOSS_DESC, JOKER_DESC, PLANET_DESC, boss_desc, consumable_desc, joker_desc,
)
...
def test_consumable_desc_dispatches_and_is_safe():
    ...
    # Tarot / Spectral return non-empty placeholders.
    assert consumable_desc(int(ConsumableKind.TAROT), 1)
...
def test_descriptions_are_short_one_liners():
    for table in (JOKER_DESC, BOSS_DESC, PLANET_DESC):
