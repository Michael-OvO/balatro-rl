...
def test_shop_offsets_map_correctly():
    ...
    floor = np.finfo(np.float32).min
    # explicit offsets from actions.py (MAX_SHOP=4, MAX_JOKERS=6: buy4 sell6 reroll1 reorder30 leave1)
    assert (actions._BUY0, actions._SELL0, actions._REROLL, actions._REORDER0, actions._LEAVE) \
        == (436, 440, 446, 447, 477)
    for off in [actions._BUY0, actions._BUY0 + 3, actions._SELL0, actions._SELL0 + 5,
